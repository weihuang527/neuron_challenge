...
class Train(Dataset):
	...
	def __getitem__(self, index):
		...
		raw_name = name[0]
		label_name = name[1]
		im = np.asarray(cv2.imread(os.path.join(self.folder_name, raw_name)))
		lb = np.asarray(cv2.imread(os.path.join(self.folder_name, label_name), cv2.IMREAD_GRAYSCALE))
...
class Valid(Dataset):
	...
	def gen(self, ids):
		...
		raw_ = raw_.astype(np.float32) / 255.0
		label_ = label_ // 255
		return raw_, label_
