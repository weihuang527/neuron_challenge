...
def loop(cfg, train_provider, valid_provider, model, criterion, optimizer, iters, writer):
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        loss2 = loss_vgg(pred_, target_)
        loss1_ = mse_weight * loss1
        loss2_ = vgg_weight * loss2
        loss = loss1_ + loss2_
        loss.backward()
        ...
        if iters % cfg.TRAIN.display_freq == 0:
            ...
            logging.info('step %d, loss1 = %.4f, loss2 = %.4f' % (iters, loss1_, loss2_))
            writer.add_scalar('final_loss', sum_loss / cfg.TRAIN.display_freq * 10, iters)
            writer.add_scalar('loss/loss1', loss1_, iters)
            writer.add_scalar('loss/loss2', loss2_, iters)
...
if __name__ == "__main__":
    ...
    if cfg.TRAIN.track == 'simple':
        ...
        from data_simple_data1 import Provider
    else:
