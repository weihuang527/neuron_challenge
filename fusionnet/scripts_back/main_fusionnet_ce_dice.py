...
def loop(cfg, train_provider, valid_provider, model, criterion, optimizer, iters, writer):
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        n,c,h,w = target_np.shape
        beta0 = np.sum(target_np==0) / (n*c*h*w) * balance_factor
        beta1 = 1 - beta0

        loss_dice = DiceLoss().to(device)
        # loss_ce = F.cross_entropy(pred, target.squeeze(1).long(), weight=torch.Tensor([beta1, beta0]).cuda())
        loss_ce = nn.CrossEntropyLoss(weight=torch.Tensor([beta1, beta0]).cuda())
        loss1 = loss_dice(pred, target.long())
        loss2 = loss_ce(pred, target.squeeze(1).long())
        ...
        if iters % cfg.TRAIN.valid_freq == 0:
            ...
            input0 = input0[0]
            im_cat = np.concatenate([input0, pred[1], target], axis=1)
