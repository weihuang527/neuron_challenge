...
class Crop_image(object):
    def __init__(self, img, crop_size=512, overlap=256):
        ...
        self.pred = np.zeros((self.d, self.h, self.w), dtype=np.float32)
        self.add_map = np.zeros((self.d, self.h, self.w), dtype=np.float32)
        self.num = (self.h - self.crop_size) // self.overlap + 1
        self.add_crop = np.ones((self.d, crop_size, crop_size), dtype=np.float32)
