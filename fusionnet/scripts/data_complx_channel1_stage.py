...
class Train(Dataset):
	...
	def __getitem__(self, index):
		...
		prediction = np.asarray(cv2.imread(os.path.join(self.folder_name, pd_name), cv2.IMREAD_GRAYSCALE))
		...
		pd = prediction[i:i+crop_size_x, j:j+crop_size_y]
		...
		pd = cv2.resize(pd, (self.crop_size[0], self.crop_size[1]), interpolation=cv2.INTER_LINEAR)
...
class Valid(Dataset):
	...
	def gen(self, ids):
		...
		raw = np.asarray(cv2.imread(os.path.join(self.folder_name, raw_name), cv2.IMREAD_GRAYSCALE))
		label = np.asarray(cv2.imread(os.path.join(self.folder_name, label_name), cv2.IMREAD_GRAYSCALE))
		pd = np.asarray(cv2.imread(os.path.join(self.folder_name, pd_name), cv2.IMREAD_GRAYSCALE))
		h, w = label.shape
