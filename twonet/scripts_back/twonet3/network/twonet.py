...
import torch
import network.unetpp 
import network.unetpp2
...
class Dual_net(nn.Module):
    def __init__(self, input_channels=1):
        ...
        self.net1 = network.unetpp.NestedUNet(input_channels=1,
                                                nb_filter = [16, 32, 64, 128, 256],
                                                layers=[12,8,6,3])
        self.out1 = self._out_block1()

        self.net2 = network.unetpp2.NestedUNet(input_channels=self.middle_planes,
                                                nb_filter = [16, 32, 64, 128, 256],
                                                layers=[3,4,6,3])
