...
class VGGBlock(nn.Module):
    ...
    def forward(self, x, flag=0):
        # if x.shape[2]<200 and flag==1:
        #     half_x = F.interpolate(x, scale_factor=0.5)
        #     q_x = F.interpolate(x, scale_factor=0.25)
        #
        #     # full resolution
        #     out = self.conv1(x)
        #     out = self.bn1(out)
        ...
        #
        #     # half resolution
        #     out = self.conv1(half_x)
        #     out = self.bn1(out)
        #     out2 = self.act_func(out)
        #     out2 = F.interpolate(out2, scale_factor=2)
        #
        #     # 4/5 resolution
        #     out = self.conv1(q_x)
        #     out = self.bn1(out)
        #     out3 = self.act_func(out)
        #     out3 = F.interpolate(out3, size=(x.shape[2],x.shape[3]))
        #
        #     out = out1+ out2 + out3
        # else:
...
class NestedUNet(nn.Module):
    def __init__(self, input_channels=1):
        ...
        # self.ResNet = ResNet(Bottleneck, [3, 4, 6, 3], groups=32, width_per_group=4)
        # self.ResNet = ResNet(Bottleneck, [3, 4, 23, 3], groups=32, width_per_group=4)

        # self.ResNet = ResNet(Bottleneck, [16, 6, 3, 2]) # best performance 0.822
        # self.ResNet = ResNet(Bottleneck, [12, 4, 3, 2], groups=32, width_per_group=4) # 0.8210
        self.ResNet = ResNet(input_channels, Bottleneck, [6, 8, 23, 6], nb_filter)
    ...
    def forward(self, input):
        ...
        # backbone no downsample
        # x1: torch.Size([1, 64, 512, 512])
        # x2: torch.Size([1, 128, 256, 256])
        # x3: torch.Size([1, 256, 128, 128])
        ...
        # FUSION
        # N0 = x0_4
        # N1 = self.fusion1(N0, x1_3)
        # N2 = self.fusion2(N1, x2_2)
        # N3 = self.fusion3(N2, x3_1)
        # N2 = N2 + self.up(self.ac1(N3))
        # N1 = N1 + self.up(self.ac2(N2))
        # N0 = N0 + self.up(self.ac3(N1))
        #
        # N0 = self.fusion_conv(N0)
        # output = F.upsample(N0, size=(x_shape[2], x_shape[3]), mode='bilinear')
