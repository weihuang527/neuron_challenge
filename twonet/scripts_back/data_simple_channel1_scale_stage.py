...
class Train(Dataset):
	def __init__(self, cfg):
		...
		assert len(self.path_list[0].split(' ')) == 3, 'Data list error!'
		...
		start = time.time()
		for fs in self.path_list:
			name = fs.split(' ')
			# self._raw_input.append(np.asarray(Image.open(os.path.join(self.folder_name, name[0]))))
			self._raw_input.append(np.asarray(Image.open(os.path.join(self.folder_name, name[0])).convert('L')))
			self._raw_labels.append(np.asarray(Image.open(os.path.join(self.folder_name, name[1]))))
			self._raw_prediction.append(np.asarray(Image.open(os.path.join(self.folder_name, name[2]))))
			# self._raw_labels.append(np.asarray(tifffile.imread(os.path.join(self.folder_name, name[1]))))
		print('End..., COST TIME: ', (time.time() - start))
		self._raw_input = np.asarray(self._raw_input)
		# self._raw_input = self._raw_input.transpose(0, 3, 1, 2)
		self._raw_labels = np.asarray(self._raw_labels)
		self._raw_prediction = np.asarray(self._raw_prediction)
	...
	def __getitem__(self, index):
		...
		i = random.randint(0, self._raw_input.shape[1] - crop_size_x)
		j = random.randint(0, self._raw_input.shape[2] - crop_size_y)
		k = random.randint(0, self.num - 1)
		
		im = self._raw_input[k, i:i+crop_size_x, j:j+crop_size_y]
		lb = self._raw_labels[k, i:i+crop_size_x, j:j+crop_size_y]
		pd = self._raw_prediction[k, i:i+crop_size_x, j:j+crop_size_y]

		# scale
		im = cv2.resize(im, (self.crop_size[0], self.crop_size[1]), interpolation=cv2.INTER_CUBIC)
		lb = cv2.resize(lb, (self.crop_size[0], self.crop_size[1]), interpolation=cv2.INTER_CUBIC)
		pd = cv2.resize(pd, (self.crop_size[0], self.crop_size[1]), interpolation=cv2.INTER_CUBIC)

		thresd = 122
		lb[lb >= thresd] = 255
		lb[lb < thresd] = 0
		lb = lb // 255
		# lb = binary_erosion(lb, iterations=1, border_value=1)
		...
		lb = lb[np.newaxis, :, :]
		pd = pd[np.newaxis, :, :]
		im_lb = np.concatenate([im, lb, pd], axis=0)
		...
		if self.random_flipz and random.uniform(0, 1) < 0.5:
			for j in range(im_lb.shape[0]): im_lb[j, :, :] = np.flip(im_lb[j, :, :], axis=0)
		...
		im = im[np.newaxis, :, :]
		pd = pd[np.newaxis, :, :]
		im = np.concatenate([im, pd], axis=0)
...
class Valid(Dataset):
	...
	def gen(self, ids):
		...
		# raw = np.asarray(np.asarray(Image.open(os.path.join(self.folder_name, raw_name))))
		# label = np.asarray(cv2.imread(os.path.join(self.folder_name, label_name), cv2.IMREAD_GRAYSCALE))
		label = np.asarray(Image.open(os.path.join(self.folder_name, label_name)))
		pd = np.asarray(Image.open(os.path.join(self.folder_name, pd_name)))
		# raw = raw.transpose(2, 0, 1)
		raw = raw.astype(np.float32) / 255.0
		pd = pd.astype(np.float32) / 255.0
		label = label.copy()
		label[label<=self.thresd] = 0
		label[label>self.thresd] = 255
		label = label // 255
		raw = raw[np.newaxis, :, :]
		pd = pd[np.newaxis, :, :]
		raw = np.concatenate([raw, pd], axis=0)
		return raw, label
...
if __name__ == '__main__':
	...
	cfg_file = 'simple_twonet_512_padding0_scale.yaml'
	...
	# print(train_provider.epoch, train_provider.iteration)
